...
client = TelegramClient(config['config']['session'], int(config['config']['api_id']), config['config']['api_hash'])
file_list = config['config']['list']
#log_level = config['config']['log_level']
days = datetime.datetime.now() - datetime.timedelta(config['config']['days_to_scrape'])
...
async def searchItems(itemList,channelList,limit=100000):
	...
	'''
	#Retrive messages that are already being forwarded to the _forward_channel_
		...
		for item in itemList:
			...
			if result.count > 0:
				for msgs in result.messages:
					...
					# the forward channel message list then forward the new search result
					if str(channel.id)[4:]+ "|" + str(msgs.id) not in messagesList:
						...
						await client.forward_messages(
							...
							messages=msgs.id)
						#update messagelist adding the new message to the existing list.
						messagesList = await getMessages()
	...
	async for dialog in client.iter_dialogs():
		if dialog.name == forward_channel:
			...
			for messages in posts.messages:
				try:
					Messages.append(str(messages.fwd_from.channel_id) + "|" + str(messages.fwd_from.channel_post))
				except: 
					# not all messages are coming from a different channel (for example: the commands)
					pass
...
async def handler(event):
	...
	if '/getConfig' in event.raw_text:
		try:
			...
			await event.reply('Current Configuration:\n' +
				...
				'	days_to_scrape: ' + str(config['config']['days_to_scrape']) + '\n\n'
				'	# Channel to be used to interact with the app and to receive posts\n'
				'	forward_channel: ' + str(config['config']['forward_channel']) + '\n\n'
				'	log_level: ' + str(config['config']['log_level']) + '\n'
				)
		except yaml.YAMLError as exc:
			print(exc)
	...
	for channel in channelList:
		...
		# If it's equal to one of the channels in the list saves which one it is.
